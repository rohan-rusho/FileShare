...
import { getAuth, createUserWithEmailAndPassword, signInWithEmailAndPassword, signOut, onAuthStateChanged, updateProfile, sendPasswordResetEmail } from 'https://www.gstatic.com/firebasejs/11.10.0/firebase-auth.js';
...
if (document.readyState === 'loading') {
    ...
    console.log('DOM already loaded, Firebase should initialize immediately');
}
...
const requiredElements = {
    ...
    forgotPasswordForm, authSwitchLink, authSwitchText, forgotPasswordLink,
    backToLoginLink, userDisplayName, logoutBtn, uploadArea,
...
const handleForgotPassword = async (e) => {
    ...
    try {
        ...
        // Convert username to email format (same as login)
        const email = `${username}@fileserver.local`;
        ...
        showToast('Password reset email sent! Check your email inbox and follow the instructions.', 'success');
        ...
        if (error.code === 'auth/user-not-found') {
            errorMessage = 'Username not found. Please check your username or sign up for a new account.';
        } else if (error.code === 'auth/invalid-email') {
            errorMessage = 'Invalid username format.';
        } else if (error.code === 'auth/too-many-requests') {
            errorMessage = 'Too many reset attempts. Please try again later.';
        } else if (error.code === 'auth/configuration-not-found') {
            errorMessage = 'Email configuration not found. Please contact administrator.';
        } else if (error.message) {
        ...
        const button = e.target.querySelector('button[type="submit"]');
        button.disabled = false;
        button.innerHTML = '<i class="fas fa-paper-plane"></i> Send Reset Email';
    }
};
...
const showForgotPasswordForm = () => {
    ...
    if (!loginForm || !registerForm || !forgotPasswordForm || !authSwitchText) {
        console.error('Forms not found for forgot password display');
        return;
    }
    ...
    authSwitchText.style.display = 'none'; // Hide the switch text
};
...
const showLoginForm = () => {
    ...
    if (!loginForm || !registerForm || !forgotPasswordForm || !authSwitchText) {
        console.error('Forms not found for login display');
        return;
    }
    ...
    authSwitchText.style.display = 'block'; // Show the switch text
    authSwitchText.innerHTML = 'Don\'t have an account? <a href="#" id="auth-switch-link">Sign up</a>';

    // Re-attach event listener to the auth switch link
    setTimeout(() => {
        const newAuthSwitchLink = document.getElementById('auth-switch-link');
        if (newAuthSwitchLink) {
            newAuthSwitchLink.addEventListener('click', (e) => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
        ...
        if (forgotPasswordLink) {
            ...
            console.log('Forgot password link event listener attached');
        } else {
        ...
        if (backToLoginLink) {
            ...
            console.log('Back to login link event listener attached');
        } else {
...
⚠️  Remember to use proper security rules in production!
    ...
    setTimeout(() => {
        if (!loadingScreen.classList.contains('hidden')) {
            console.log('Loading screen timeout reached, forcing show auth screen');
            ...
            console.log('Auth screen should now be visible');
        }
    }, 3000);

    // Additional immediate check in case Firebase is already initialized
    setTimeout(() => {
        if (!loadingScreen.classList.contains('hidden')) {
            console.log('Quick check: Loading screen still visible after 1 second, checking Firebase auth state...');
            const currentAuth = auth.currentUser;
            console.log('Current auth user:', currentAuth);

            if (!currentAuth) {
                console.log('No user found, showing auth screen immediately');
                loadingScreen.classList.add('hidden');
                authContainer.classList.remove('hidden');
                mainContainer.classList.add('hidden');
            }
